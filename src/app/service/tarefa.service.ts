...
export class TarefaService {
  ...
  private tarefasSubject = new BehaviorSubject<Array<Tarefa>>([]);
  ...
  listar(): void {
    ...
    this.http.get<Tarefa[]>(this.API, { params }).subscribe(
      (tarefas) => {
  ...
  criar(tarefa: Tarefa): void {
    this.http.post<Tarefa>(this.API, tarefa).subscribe(
      (novaTarefa) => {
  ...
  editar(tarefa: Tarefa, atualizarSubject: boolean): void {
    ...
    this.http.put<Tarefa>(url, tarefa).subscribe(
      (tarefaEditada) => {
        if (atualizarSubject) {
          const tarefas = this.tarefasSubject.getValue();
